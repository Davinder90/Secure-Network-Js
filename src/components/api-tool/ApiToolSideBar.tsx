...
export const SideBarCollectionItem = ({
  ...
  const toggleFavorite = async () => {
    const resp = await handleUpdateCollection({
      ...collection,
      favorite: !favorite,
    });
    if (resp.success) {
  ...
  const generateApiDefaultName = async () => {
    ...
    const existing = apiToolState.stateApis.filter(
      (api) => api.name && api.name.startsWith(baseName),
    );
  ...
  const handleAddApiToContext = (api: ApiDocumentWithOptionalId) => {
    ...
    if (!exists) {
      dispatch(
        updateStateApis({ activeApiId: "", activeIndex: -1, data: api }),
      );
    }
  };
      ...
      <div className="flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-gray-800">
        <div className="flex items-center gap-2">
          ...
          {renaming ? (
            ...
            <span
              className={`text-sm ${favorite ? "text-yellow-400" : "text-gray-200"}`}
            >
              {collection.name}
            </span>
          )}
        ...
        <div className="flex items-center gap-1 relative">
          <button
            onClick={handleAddNewApiToContext}
          ...
          <button
            ...
            className={`transition ${favorite ? "text-yellow-400" : "text-gray-400 hover:text-yellow-400"}`}
          >
            <FiStar />
          </button>
          <button
          ...
          <AnimatePresence>
            {menuOpen && (
              ...
              >
                ...
                <button
                  onClick={() => {
                    onRun(collection._id as string);
                    setMenuOpen(false);
                  }}
                  className="w-full text-left px-3 py-2 hover:bg-gray-800"
                >
                ...
                <button
                  onClick={() => {
                    onDelete(collection._id as string);
                    setMenuOpen(false);
                  }}
...
const Sidebar = ({
  ...
  return (
    <div
      className="w-80 bg-[#121212] text-gray-200 border border-gray-800 flex flex-col"
      style={{
        position: "fixed",
        top: NAVBAR_HEIGHT,
        left: 0,
